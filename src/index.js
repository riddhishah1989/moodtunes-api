...
import camelcaseKeys from "camelcase-keys"; // ← ADD THIS
import { connectDB } from "./config/database.js";
import { verifyEmailConnection } from "./config/emailService.js";
...
// ── Auto-convert all response keys to camelCase ───────────────
// MUST be before routes so it applies to all responses
app.use((req, res, next) => {
  const originalJson = res.json.bind(res);
  res.json = (data) => originalJson(camelcaseKeys(data, { deep: true }));
  next();
});
